<div class="space-y-6">
    ...
    <div class="max-w-xl">
        <form
            ...
            id="formKategori"
            method="POST"
            action="{{ route('admin.bintaran.kategori.store') }}"
        >
            @csrf
    ...
    <div class="space-y-2">
        ...
        <div class="flex items-center justify-between p-3 bg-white border rounded-lg">
            ...
            <div class="flex gap-3">
                ...
                <button
                    type="button"
                    onclick="editKategori('{{ $item->id }}','{{ $item->nama_kategori }}','{{ $item->slug }}')"
                ...
                <form
                    id="delete-{{ $item->id }}"
                    action="{{ route('admin.bintaran.kategori.destroy', $item->id) }}"
                    method="POST"
                >
                    @csrf
                    @method('DELETE')
...
<script>
    ...
    function editKategori(id, nama, slug) {
        ...
        document.getElementById('formKategori').action =
            `/admin-tulisan-bintaran/kategori/update/${id}`
